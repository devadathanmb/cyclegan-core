...
train_dataloader = DataLoader(
    ImageDataset(
        root_path, mode=hp.dataset_train_mode, transforms_=transforms_, unaligned=False
    ),
...
val_dataloader = DataLoader(
    ImageDataset(
        root_path, mode=hp.dataset_test_mode, transforms_=transforms_, unaligned=False
    ),
    batch_size=16,
    shuffle=True,
    num_workers=1,
...
def save_img_samples(batches_done):
    ...
    path = "/content/drive/MyDrive/new_train" + "/%s.png" % (
        batches_done
...
def save_checkpoint(epoch, batches_done):
    ...
    checkpoint_path = f"/content/drive/MyDrive/new_train/latest.pth"
...
log_file_path = "/content/drive/MyDrive/new_train/train.log"


def log_to_file(log_string):
